// https://www.codechef.com/problems-old/FAMTREE
#include <bits/stdc++.h>
#define ll long long int
#define mod 1000000007
#define inf (long long int)1e18
#define log(args...)    { std::string _s = #args; replace(_s.begin(), _s.end(), ',', ' '); std::stringstream _ss(_s); std::istream_iterator<std::string> _it(_ss); err(_it, args); }
#define logarr(arr,a,b) for(int z=(a);z<=(b);z++) std::cout<<(arr[z])<<" ";std::cout<<std::endl;
#define vll std::vector<long long int>
#define vi std::vector<int>
#define vb std::vector<bool>
#define pii std::pair<int, int>
#define pllll std::pair<long long int, long long int>
#define loop(i, a, b) for(int i=a;i<=b;i++)
#define rloop(i, a, b) for(int i=a;i>=b;i--)
#define iter_all(el, cont) for(auto &el:cont)

void err(std::istream_iterator<std::string> it) {}
template<typename T, typename... Args>
void err(std::istream_iterator<std::string> it, T a, Args... args) {
	std::cout << *it << " = " << a << std::endl;
	err(++it, args...);
}

void file_i_o() {
	std::ios_base::sync_with_stdio(0);
	std::cin.tie(0);
	std::cout.tie(0);
#ifndef ONLINE_JUDGE
	freopen("input.txt", "r", stdin);
	freopen("output.txt", "w", stdout);
#endif
}

std::vector<std::vector<int>> graph;
std::unordered_map<int, std::pair<int, int>> dp;

int ans = INT_MIN;

std::pair<int, int> find_maxmin(int src, std::vector<int> &wt, int parent = -1) {

	bool leaf = true;
	std::pair<int, int> res = {INT_MAX, INT_MIN};
	for (auto &n : graph[src]) {
		if (n == parent) continue;
		leaf = false;
		std::pair<int, int> temp = find_maxmin(n, wt, src);
		res.first = std::min(res.first, temp.first);
		res.second = std::max(res.second, temp.second);
	}
	if (not leaf) ans = std::max({ans, abs(wt[src] - res.first), abs(wt[src] - res.second)});
	return {std::min(res.first, wt[src]), std::max(res.second, wt[src])};
}

int main(int argc, char const *argv[]) {
	clock_t begin = clock();
	file_i_o();
	// Write your code here....

	int n, src = -1;
	std::cin >> n;
	std::vector<int> wt(n + 1), p(n + 1);
	for (int i = 1; i <= n; i++) std::cin >> wt[i];
	for (int i = 1; i <= n; i++) std::cin >> p[i];
	graph.resize(n + 1, std::vector<int>());
	for (int i = 1; i <= n; i++) {
		if (p[i] == -1) {src = i; continue;}
		graph[p[i]].push_back(i);
		graph[i].push_back(p[i]);
	}

	find_maxmin(src, wt);

	std::cout << ans << "\n";

#ifndef ONLINE_JUDGE
	clock_t end = clock();
	std::cout << "\n\nExecuted In: " << double(end - begin) / CLOCKS_PER_SEC * 1000 << " ms";
#endif
	return 0;
}
